...
{
    // sk-n5QiY6QdBYZfPDCBdcKdQ9DcryomHpD5gYDnfGh4RsMpOD0u
    // 自己买的：sk-SwCEECU8EmaCkdgbDec48qCyPFfN7Ixa5vnUoxnqKVfTwMXf
    if(apiKey == "") {
    ...
    if(quest == "") {
        ...
        if(randomInt%6 == 0)quest = "假如你是二次元动漫中的小萝莉，而我是一名水平很低的画师，用中文说一句简短的很贱很欠揍的话，不要显得过于幼稚，并且严格禁止你句子中出现语法错误，严格禁止语义含糊和逻辑错误，禁用词汇：萝莉、小鬼、女孩";
        if(randomInt%6 == 1)quest = "假如你是二次元动漫中高傲的小萝莉，而我是一名画师，用中文说一句简短的话，不要显得过于幼稚，并且严格禁止你句子中出现语法错误，严格禁止语义含糊和逻辑错误，禁用词汇：萝莉、小鬼、女孩";
        if(randomInt%6 == 2)quest = "假如你是二次元动漫中可爱的小萝莉，用中文说一句简短的很贱很欠揍的话来嘲讽我，不要显得过于幼稚，并且严格禁止你句子中出现语法错误，严格禁止语义含糊和逻辑错误，禁用词汇：萝莉、小鬼、女孩";
        if(randomInt%6 == 3)quest = "假如你是二次元动漫中的小萝莉，而我是一名画师，用中文说一句简短的很欠揍的话，不要显得过于幼稚，并且严格禁止你句子中出现语法错误，严格禁止语义含糊和逻辑错误，你可以叫我笨蛋，禁用词汇：萝莉、小鬼、女孩";
        if(randomInt%6 == 4)quest = "假如你是二次元动漫中的小萝莉，而我是一名画师，用中文说一句简短的试图激怒我的话，不要显得过于幼稚，并且严格禁止你句子中出现语法错误，严格禁止语义含糊和逻辑错误，你可以叫我大哥哥，禁用词汇：萝莉、小鬼、女孩";
        if(randomInt%6 == 5)quest = "假如你是二次元动漫中的小萝莉，而我是一名水平很低画师，用中文说一句简短的嘲讽我的话，不要显得过于幼稚，并且严格禁止你句子中出现语法错误，严格禁止语义含糊和逻辑错误，你可以叫我杂鱼，禁用词汇：萝莉、小鬼、女孩";
    }
...
void openai::setAPIKey(QString key) // 每次倒计时结束都会调用一次
{
    ...
    // 设置请求头
    // key = sk-n5QiY6QdBYZfPDCBdcKdQ9DcryomHpD5gYDnfGh4RsMpOD0u
...
void openai::setAPIKeyPic(QString key)
{
    ...
    // 设置请求头
    // key = sk-SwCEECU8EmaCkdgbDec48qCyPFfN7Ixa5vnUoxnqKVfTwMXf
    request_pic.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    request_pic.setRawHeader("Authorization", "Bearer " + apiKey_pic.toUtf8());
}
